...
type
  ...
  TFoolTrayIcon = class(TComponent)
  private
    FEnabled: Boolean;
    FIcon: TIcon;
...
constructor TFoolTrayIcon.Create(AOwner: TComponent);
begin
...
destructor TFoolTrayIcon.Destroy;
begin
...
procedure TFoolTrayIcon.Loaded;
...
procedure TFoolTrayIcon.Notification(AComponent: TComponent;
...
{ For MinimizeToTray to work, we need to know when the form is minimized
  ...
  The straight-forward way is to make TFoolTrayIcon trap the
  Application.OnMinimize event. However, if you also make use of this
  event in the application, the OnMinimize code used by TFoolTrayIcon
  is discarded.
...
procedure TFoolTrayIcon.HookApp;
begin
...
procedure TFoolTrayIcon.UnhookApp;
begin
...
procedure TFoolTrayIcon.HookAppProc(var Msg: TMessage);
begin
  case Msg.Msg of
    WM_SIZE:
...
procedure TFoolTrayIcon.HookParent;
begin
...
procedure TFoolTrayIcon.UnhookParent;
begin
...
procedure TFoolTrayIcon.HookWndProc(var Msg: TMessage);
begin
  case Msg.Msg of
...
procedure TFoolTrayIcon.HandleIconMessage(var Msg: TMessage);
...
procedure TFoolTrayIcon.SetIcon(Value: TIcon);
begin
  FIcon.Assign(Value);
  ModifyIcon;
end;
...
procedure TFoolTrayIcon.SetIconVisible(Value: Boolean);
begin
  if Value then
    ShowIcon
  else
...
procedure TFoolTrayIcon.SetDesignPreview(Value: Boolean);
begin
...
procedure TFoolTrayIcon.SetCycleIcons(Value: Boolean);
begin
  FCycleIcons := Value;
  if Value then
    SetIconIndex(0);
...
procedure TFoolTrayIcon.SetCycleInterval(Value: Cardinal);
begin
...
procedure TFoolTrayIcon.SetIconList(Value: TImageList);
begin
  FIconList := Value;
{
...
procedure TFoolTrayIcon.SetIconIndex(Value: Integer);
begin
...
procedure TFoolTrayIcon.SetHint(Value: String);
begin
  FHint := Value;
  ModifyIcon;
end;
...
procedure TFoolTrayIcon.SetShowHint(Value: Boolean);
begin
  FShowHint := Value;
  ModifyIcon;
end;
...
function TFoolTrayIcon.InitIcon: Boolean;
...
function TFoolTrayIcon.ShowIcon: Boolean;
...
function TFoolTrayIcon.HideIcon: Boolean;
...
function TFoolTrayIcon.ModifyIcon: Boolean;
...
procedure TFoolTrayIcon.TimerCycle(Sender: TObject);
begin
...
procedure TFoolTrayIcon.ShowMainForm;
begin
...
procedure TFoolTrayIcon.HideMainForm;
begin
...
procedure TFoolTrayIcon.Refresh;
// Refresh the icon
begin
  ModifyIcon;
end;
...
procedure TFoolTrayIcon.PopupAtCursor;
var
  CursorPos: TPoint;
  I: Integer;
  Found: Boolean;
...
procedure TFoolTrayIcon.DblClick;
begin
  // Execute user-assigned method
...
procedure TFoolTrayIcon.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  // Execute user-assigned method
...
procedure TFoolTrayIcon.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  // Execute user-assigned method
...
procedure TFoolTrayIcon.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  // Execute user-assigned method
...
procedure TFoolTrayIcon.CycleIcon;
var
...
procedure TFoolTrayIcon.DoMinimizeToTray;
begin
...
procedure Register;
begin
  RegisterComponents('Custom', [TFoolTrayIcon]);
end;
